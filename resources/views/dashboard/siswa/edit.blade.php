...
@section('content')
<form class="row g-3 col-12 col-lg-8" action="/siswa/{{ $data->id }}" method="POST">
    @csrf @method('put')
    <div class="col-md-6">
        <label for="nisn" class="form-label">NISN</label>
        <input type="number" class="form-control @error('nisn') is-invalid @enderror" id="nisn" name="nisn"
            value="{{ old('nisn', $data->nisn) }}" required autofocus>
        @error('nisn')
    ...
    <div class="col-md-6">
        ...
        <input type="text" class="form-control @error('nama_lengkap') is-invalid @enderror" id="nama_lengkap"
            name="nama_lengkap" value="{{ old('nama_lengkap', $data->nama_lengkap) }}" required>
        @error('nama_lengkap')
        <div class="invalid-feedback">
            {{ $message }}}
        </div>
